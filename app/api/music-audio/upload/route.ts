...
function requireAdminSession(
  ...
  const currentToken = request.cookies
  ...
  if (
    ...
    throw new Error(
      "No tienes autorización para gestionar audios.",
    );
  }
}
...
function parseRegisterAction(
  ...
  if (
    ...
    throw new Error(
      "El tamaño del audio no es válido.",
    );
  }
  ...
  return {
    ...
    url: requiredText(
      body.url,
      "La dirección del audio no es válida.",
    ),
    pathname: requiredText(
      body.pathname,
      "La ubicación del audio no es válida.",
    ),
    fileName: requiredText(
      body.fileName,
      "El nombre del audio no es válido.",
    ),
    contentType:
...
async function registerAudio({
  ...
  if (!ALLOWED_EXTENSIONS.has(extension)) {
    throw new Error(
      "El formato del audio no es válido.",
    );
  }
  ...
  try {
    ...
    try {
      ...
      console.error(
        "VANMOTION_MUSIC_AUDIO_CLEANUP_ERROR:",
        cleanupError,
      );
    }
    throw error;
  ...
  if (
    track.fileUrl !== url &&
    isVercelBlobUrl(track.fileUrl)
  ) {
    try {
      await del(track.fileUrl);
    } catch (deleteError) {
      console.error(
        "VANMOTION_OLD_MUSIC_AUDIO_DELETE_ERROR:",
        deleteError,
      );
    }
  }
...
export async function POST(
  ...
  try {
    ...
    if (registerAction) {
      ...
      await registerAudio({
        trackId: registerAction.trackId,
        url: registerAction.url,
        pathname: registerAction.pathname,
      });
    ...
    const response = await handleUpload({
      ...
      onUploadCompleted: async ({
        ...
        try {
          ...
          console.error(
            "VANMOTION_MUSIC_AUDIO_COMPLETION_ERROR:",
            completionError,
          );
        }
      },
